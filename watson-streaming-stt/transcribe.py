...
def Chatbot_Msg(txt):
        # For Chtabot Authenticator 
        authenticator = IAMAuthenticator('ju1vlaHd7gdyW-qY5OtLgPNTLNB9Ov_NOecZIdxdFHc5')
        assistant = AssistantV2(
            version='2021-07-13',
            authenticator = authenticator
        )
        assistant.set_service_url('https://api.eu-de.assistant.watson.cloud.ibm.com/instances/e06a0095-2985-49df-8a9b-55ed1ea4a8de')
        
        # Save User Input (Speech) as .txt File (STT)
        with open("UserInput.txt", "w") as file:
            file.write(txt)

        # Send User Input to Assistant As Text ( from .txt File)
        UserInput = assistant.message_stateless(
        assistant_id = '6000bd11-5d91-4e74-97ef-e93e5aebd555',
        input = {
            'message_type': 'text',
            'text': txt
        }
        ...
        # Get Response from Assistant As Text
        Ans = json.loads(json.dumps(UserInput, indent=2)) 
        chatpotRes = Ans["output"]["generic"][0]["text"]
        print(chatpotRes) 

        # Save Response (Text) as .txt File 
        with open("chatpotResponse.txt", "w") as file:
            file.write(chatpotRes)
        

def Get_TextToSpeech():
        # For Text to Speech
        url = 'https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/af4b55b8-4db4-47a0-8e05-5c3c22a79821'
        APIKEY = 'e5BDpA5PzgqtFeM8r0_qR9SC0-6djFH2NKD200DRntmy'
        # Setup Services
        ...
        # Save Response (Text) as .mp3 File (TTS)
        with open('chatpotResponse.txt', 'r') as f:
            text = f.readlines()
        text = [line.replace('\n','') for line in text]
